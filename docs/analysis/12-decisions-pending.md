...
[48-player-self-update.md](48-player-self-update.md) 7절에 적었다.

| | 무엇 | 상태 |
|---|---|---|
| D-P1 | 윈도우 자동 교체 | **✅ 2026-09-04 완료** — 도우미 + 60초 생존 확인 + 되돌림 (48번 문서 10절, 되돌림 실동 검증) |
| D-P2 | 리눅스 `.deb` 설치 | **✅ 완료** — 경로 좁힌 sudoers + 패키지명 검증 도우미. 실장비 1회 확인 남음 |
| D-P3 | 원격 업그레이드 지시 | **✅ 완료** — SignalR UpdateNow + `POST /building/device/update-now/{code}`. 포털 단추는 장비 화면 개편 뒤 |
| D-P4 | 주기 확인 · 자동 설치 | **주기 확인+버전 보고까지 완료.** 자동 설치는 안 함 — 장례 중 재시작 금지 판정이 먼저다 |
...
## D-M1 ~ D-M4. 장례 API 가 익명으로 열려 있다 (2026-09-03)

플레이어 영정사진이 안 보이던 것을 고치다 함께 확인했다
([46번 문서](46-player-media-anonymous-access.md) 6절).

게이트웨이에 플레이어용 익명 라우트가 여섯 개 있고 **의도한 것**이다. 문제는 열쇠가
**장비 코드 하나**이고 `JSI-06-0001` 처럼 추측 가능하다는 점이다. 맞히면 고인 성명·
상주 이름·사진 URL 이 인터넷에서 그대로 조회되고, 응답 스키마에는 `ssn`·상주 연락처·
주소·계약자 칸까지 있다.

**지금 피해는 작다** — 실제 자료가 고인 2건이고 `ssn` 은 한 건도 채워져 있지 않다.
현장이 늘면 그대로 커진다.

| | 무엇 | 상태 |
|---|---|---|
| D-M2 | 익명 응답에서 표출에 쓰지 않는 칸 제거(`ssn`·`causeOfDeath`·`burialPlot`·비고·계약자·장례지도사·상주 연락처/주소/이메일·장비 IP/MAC/공인IP) | **✅ 2026-09-03 완료** (46번 문서 7절) |
| D-M3 | 익명 라우트에 IP 당 요청 제한 | **✅ 2026-09-03 완료** — 정책 `player-read`, IP 당 분당 900회 (46번 문서 8절) |
| D-M1 | 장비마다 임의 비밀(등록 토큰) 발급 후 플레이어가 함께 전송 | **기반 구현됨 (2026-09-03, 기본 꺼짐)** — 게이트웨이 `PlayerAuth` 스위치 + 플레이어 설정의 장비 인증 키 칸(46번 문서 9절). **켜는 결정**과 장비별 DB 토큰(47번 개편 커밋 뒤)이 남았다 |
| D-M4 | 그대로 둔다(내부망 전제) | 지금 공개 도메인이라 전제가 성립하지 않는다 |

권장 순서는 **D-M2 → D-M3 → D-M1** 이었고 **D-M2 · D-M3 을 했다. 남은 것은 D-M1 뿐이다.**
둘 다 플레이어를 고치지 않으므로 **현장 장비 갱신은 필요 없다**
(축약 응답은 플레이어 파서로 실제로 읽혀 확인했다).

D-M3 의 한도 900 은 **장례식장 단위**로 잡은 값이다 — 한 건물의 화면이 NAT 뒤에서
공인 IP 하나를 공유한다(실측: 장비 10대 중 8대가 IP 하나). 정전 복구로 화면 40개가
동시에 켜지는 순간(최대 600회)을 넘기기 위한 것이고, **코드 대입 자체는 못 막는다** —
그것이 D-M1 이다.

**파일 접근은 결정이 나서 이미 고쳤다** — `Files:AnonymousReadablePaths` 에
`funeralv2/` 만 허용한다(46번 문서 3절). 이 변경은 위 노출을 넓히지 않는다.
