...
## D-P1 ~ D-P4. 플레이어 새 버전 업그레이드 (2026-09-02)

환경 설정 화면에 **새 버전 확인**을 넣었다(전 플랫폼). **실제 교체는 안드로이드만** 넣었다 —
시스템 설치 화면을 띄우는 길이 열려 있어서다.

데스크톱은 받기까지만 한다. 결정이 필요해서 남긴 것 넷이고,
각각 ①지금 상태 ②왜 결정인가 ③선택지 ④내 의견 순으로
[44-player-self-update.md](44-player-self-update.md) 7절에 적었다.

| | 무엇 | 내 의견 |
|---|---|---|
| D-P1 | 윈도우에서 앱이 스스로 exe 를 교체할지 | 도우미 스크립트 + **되돌림**까지 있으면 하고, 없으면 지금처럼 |
| D-P2 | 리눅스에서 앱이 `.deb` 를 설치할 수 있게 할지 (root 필요) | 길게는 자체 apt 저장소, 당장은 경로를 좁힌 sudoers 한 줄 |
| D-P3 | 업그레이드를 현장에서 누를지, 포털에서 원격으로 밀지 | 둘 다 (플레이어는 이미 SignalR 로 붙어 있다) |
| D-P4 | 주기 확인 · 자동 설치를 할지 | 주기 확인 + 알림까지. **장례 중 재시작은 안 된다** |

지금 상태로도 동작에 빠진 것은 없다 — 확인은 되고, 받는 것도 되고,
데스크톱은 받은 파일 위치와 설치 명령을 화면에 적어 준다.
