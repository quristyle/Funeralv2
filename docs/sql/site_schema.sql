...
--
-- ⚠ **어느 DB 에 돌리는가 — `jsinisite` 다.**
--   이 폴더의 다른 SQL 은 대부분 `jsiniportal` 용이라 헷갈리기 쉽다.
--   소개 사이트는 전용 DB 를 쓴다(docs/sql/site_schema_move_to_own_db.sql).
--
-- 왜 DB 를 따로 쓰나
--   **SiteServer 만 로그인하지 않은 사람의 입력을 받기 때문**이다(문의 접수).
--   익명 쓰기가 닿는 표와 업무 표가 같은 DB 에 있으면, 경계가 DB 권한이 아니라
--   코드 안에만 있게 된다. 나눠 두면 그 경계가 물리적으로 생긴다.
--   (처음에는 funeralv2 안에 스키마만 나눠 두었다가 옮겼다)
--
-- 왜 EF 마이그레이션이 아닌가
