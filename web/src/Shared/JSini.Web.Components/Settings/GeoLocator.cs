    ...
    ILogger<GeoLocator> logger)
{
    ...
    /// </summary>
    /// <remarks>
    /// 알림이 가는 시각이 보통 아침·저녁 둘이라(기본값 <c>7,18</c>) 세 시간이면
    /// 자리가 바뀐 것이 그날 안에 따라온다. 더 자주 재도 얻는 것이 없고,
    /// 휴대폰에서는 잴 때마다 배터리를 쓴다.
    /// </remarks>
    public static readonly TimeSpan SyncInterval = TimeSpan.FromHours(3);
