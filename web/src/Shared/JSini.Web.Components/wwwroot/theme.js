...
(function () {
   ...
   * [무엇에 쓰나]
   *
   * 휴대폰에서 사이드바는 본문을 덮는다. 덮개가 떠 있을 때 뒤로 가기는
   * **화면을 옮기기 전에 덮개부터 걷는 것**이 안드로이드의 오래된 약속이고,
   * 포털은 홈 화면에 설치되어 기기의 뒤로 가기 단추를 그대로 받는다.
   *
   * 레이아웃(`MainLayout`)은 이동을 가로챌 수 있다
   * (`RegisterLocationChangingHandler`). 그런데 **그 자리에서는 이 이동이
   * 뒤로 가기인지 메뉴를 누른 것인지 가릴 수 없다** — 둘이 같은 모습으로
   * 온다. 그래서 뒤로 가기만 여기서 세어 두고, 레이아웃은 메뉴를 펼 때
   * 그 수를 적어 두었다가 이동할 때 다시 묻는다. **늘었으면 뒤로 가기다.**
   *
   * [히스토리에 되돌림 자리를 밀어 넣지 않는 이유]
   *
   * 흔한 방법은 메뉴를 펴면서 `pushState` 로 자리를 하나 만들어 두고 그것이
   * 빠지는 것을 보는 것이다. 여기서는 둘 다 안 된다.
   *
   *   · JS 로 직접 밀어 넣으면 **Blazor 의 셈이 어긋난다.** 프레임워크는
   *     제가 만든 자리에 번호(`_index`)를 매겨 두고 뒤로 가기가 오면 그
   *     차이만큼 `history.go()` 로 되돌린다. 우리 자리에는 번호가 없어
   *     차이가 0 이 되고, `history.go(0)` 은 **새로고침**이다.
   *   · `NavigationManager` 로 밀어 넣으면 번호는 맞지만 **같은 주소로 한 번
   *     옮겨 간 것**이 되어 라우터가 화면을 다시 그린다. 메뉴를 여닫을
   *     때마다 목록을 다시 읽는 화면이 생긴다.
   *
   * 세기만 하면 히스토리를 건드리지 않는다. 대신 그 뒤로 가기 한 번은
   * **삼켜진다** — 메뉴만 닫히고 화면은 그대로다. 한 번 더 누르면 옮겨 간다.
   * 안드로이드의 서랍이 그렇게 움직인다.
   *
   * [프레임워크가 되돌리는 것까지 세어도 된다]
   *
   * Blazor 는 뒤로 가기를 받으면 서버에 묻기 전에 `history.go(1)` 로 **한 번
   * 되돌려 놓는다**(막을 수 있어야 하므로). 그래서 한 번 누르면 이 수가 둘씩
   * 오른다 — 우리는 늘었는지만 보므로 상관없다.
   */
  var pops = 0;

  window.addEventListener('popstate', function () { pops += 1; });
  ...
  // ── 잠금화면의 표시를 지운다 ────────────────────────────────
  //
  // 잠금 표시는 **기기에 남는다**(`PortalBoot.ScreenLockedKey` 머리말).
  // 그래야 PWA 앱을 껐다 켜도 덮개가 그대로인데, 대신 걷어 줄 길목이
  // 하나 필요하다 — 그 길목이 **로그인 화면**이다.
  //
  // 로그인 화면을 본다는 것은 지금 잠겨 있지 않다는 뜻이고(쿠키가 없거나
  // 방금 로그아웃했다) 곧 비밀번호를 칠 참이다. 안 지우면 잠금화면에서
  // 로그아웃한 사람이 같은 비밀번호를 연달아 두 번 치게 된다.
  //
  // **열쇠를 여기 적지 않는다.** 정본은 서버(`PortalBoot`)이고 로그인
  // 화면이 마크업으로 넘겨 준다 — `jsiniNotice` 가 닫힘 표시 열쇠를
  // 받는 것과 같다. 양쪽에 적으면 한쪽만 고치는 날이 오고, 그때 증상은
  // 「로그인했는데 잠금화면이 뜬다」다.
  window.jsiniLock = {
    /**
     * 잠금 표시를 지운다. **못 지워도 던지지 않는다** — 사생활 보호
     * 모드에서는 저장소에 손대는 것만으로 던지는데, 그 자리에서 던지면
     * 같은 인라인 <script> 의 뒷줄(소셜 링크 처리)이 함께 죽는다.
     *
     * @param {string} key `PortalBoot.ScreenLockedKey`
     */
    forget: function (key) {
      if (!key) return;

      try { window.localStorage.removeItem(key); } catch (e) { /* 위와 같다 */ }

      // 옛 자리(세션)에 남은 것도 함께 지운다. 안 지우면 옮기기 전에
      // 잠가 둔 탭이 그 표시를 계속 들고 있는데, 읽는 쪽은 이제 로컬만
      // 보므로 **지우지 않아도 증상은 없다** — 쓰레기를 안 남기려는 것뿐이다.
    ...
    init: function (form) {
      ...
      if (saved) {
        ...
        if (!id.value) {
          ...
          // **사람이 친 값과 갈라 둔다.** 지문 단추는 이 값을 쓰지 않는다 —
          // 이것은 지난번에 *비밀번호로* 들어온 사람의 아이디라, 다른 사람이
          // 잠깐 로그인하고 나간 브라우저에서는 **내 지문이 아니라 그 사람의
          // 열쇠**를 찾게 되고 그 사람에게 패스키가 없으면 반드시 실패한다
          // (passkey.js 의 `typedUsername`).
          id.dataset.jsiniAutofilled = '1';
        }
      }

      // 칸을 건드리는 순간 그 값은 사람이 고른 값이다. 지우고 다시 쳐도,
      // 브라우저 자동완성으로 채워도 이 사건이 온다.
      id.addEventListener('input', function () {
        delete id.dataset.jsiniAutofilled;
      });
