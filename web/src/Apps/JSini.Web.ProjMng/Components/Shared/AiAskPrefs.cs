...
/// 「보내고 올리기까지」로 바뀐다(누르기 직전에 눈이 닿는 자리다). 허용하지
/// 않는 대상으로 옮기면 저절로 꺼져 보인다(<c>AiAsk.PushOn</c>).
/// 누른 뒤에 묻는 창은 없다 — 까닭은 <c>AiAskPanel</c> 머리말에 적었다.
/// </para>
///
/// <para>
/// [왜 서버가 아니라 브라우저인가]
...
public sealed class AiAskPref
{
    ...
    /// <summary>
    /// 아래 카드를 <b>몇 단으로 깔 것인가</b>(1 · 2 · 3).
    /// <b>기억이 없으면 둘</b>이다 — 한동안 그것 하나뿐이었다.
    /// </summary>
    /// <remarks>
    /// 이 값이야말로 <b>기기마다 다른 것이 맞는</b> 값이다. 세로로 쥔 휴대폰에
    /// 세 단을 깔면 제목이 두 낱말 만에 잘리고, 책상의 넓은 화면에 두 단을
    /// 깔면 카드가 옆으로 늘어져 오른쪽이 허옇게 남는다. 그래서 여기 있다 —
    /// 서버에 담으면 한쪽에서 바꾼 것이 다른 쪽을 따라온다(위 머리말).
    /// <para>
    /// 범위를 넘는 값이 들어와도 화면은 멀쩡해야 한다. 읽는 쪽이
    /// <c>AiAskPanel</c> 에서 1~3 으로 맞춘다 — 손으로 고친 저장소나 옛 판이
    /// 남긴 0 이 <b>단이 0개인 격자</b>가 되면 카드가 통째로 안 그려진다.
    /// </para>
    /// </remarks>
    public int ListCols { get; set; } = 2;
