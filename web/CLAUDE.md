...
### 셸 껍데기에 **모듈의 화면 조각**을 실어야 할 때 (2026-09-24)

규칙 4번(`Components` → 업무 모듈 참조 금지) 때문에 레이아웃은 모듈의 부품을
이름으로 쓸 수 없다. 그런데 실제로 그것이 필요한 자리가 생겼다 — 헤더의 ⚡ 가
여는 「빠른 지시」 서랍(`QuickAskDrawer`)이 그린 것은 프로젝트관리 모듈의
`AiAskPanel` 이다.
...
| 누가 | 무엇을 |
|---|---|
| `Components` | 알맹이를 담는 통(`QuickAskContent`)과 서랍을 둔다. 타입은 모른다 |
| 업무 모듈 | `ConfigureServices` 에서 **자기 부품의 `Type` 을 등록**한다 |
| 서랍 | `DynamicComponent` 로 그것을 그린다 |

셸이 어셈블리를 훑어 `IPortalModule` 을 찾는 것과 같은 구도다 — 컴파일 시점에는
서로의 이름을 모르고, 모듈을 빼면 **그 자리가 조용히 없어질 뿐** 빌드가 깨지지
않는다(`QuickAskReveal.CanDock` 이 거짓이 되고 단추는 옛 길인 화면 이동으로
돌아간다).

> 알맹이를 **화면과 나눠 쓰면 `@page` 와 `<PageTitle>` 은 화면 쪽에 남긴다.**
> 서랍은 한 번 열면 계속 살아 있어서, 제목을 알맹이에 걸면 그 뒤로 브라우저 탭
> 이름이 그 화면 이름에 붙박인다. `AiAsk` ↔ `AiAskPanel` 이 그렇게 갈라져 있다.

> 권한 판정도 갈린다. `PermissionView` 는 경로를 안 주면 **지금 열려 있는 화면**
> 으로 따지는데, 서랍 안에서 그것은 사람이 보던 남의 화면이다.
> 나눠 쓰는 알맹이는 `Path` 를 **제 화면 경로로 박아 준다.**
...
### 로그인 방식 우선순위는 **계정이 아니라 브라우저**에 있다

「내 정보 → 보안 설정」에서 고른다(비밀번호 먼저 · 지문·얼굴 먼저). 지문으로
두면 로그인 화면이 뜨는 순간 기기 확인이 열리고 그 판이 폼 위로 올라간다.

| | 어디에 | 누가 읽나 |
|---|---|---|
| 등록한 패스키 | 서버 (`scom.account_webauthn_credentials`) | 게이트웨이 → AuthServer |
| **우선순위** | **브라우저 localStorage** (`AUTH_PRIORITY_<호스트>`) | `passkey.js` 뿐 (잠금화면은 `status` 로 받아 본다) |
| **지문으로 들어가는 아이디** | **브라우저 localStorage** (`PASSKEY_USERNAME_<호스트>`) | `passkey.js` 뿐 |

서버에 두지 않는 이유가 둘이다. **읽어야 하는 곳이 로그인 화면**인데 거기는
로그인 전이라 서버가 누구인지 모르고(아이디를 실어 물으면 「이 계정은 지문을
쓴다」가 밖으로 샌다), **패스키 자체가 기기마다 따로**라 계정에 적으면 등록한
적 없는 기기까지 따라 하다 반드시 실패한다.

그래서 화면(`Login.razor`)이 하는 일은 담는 통(`.jsini-login__methods`) 하나를
두는 것뿐이고, 차례를 바꾸는 것은 CSS, 값을 읽고 기기를 부르는 것은
`passkey.js` 다. **자동으로 안 여는 자리가 넷 있다**(`shouldAutoStart`) —
실패 안내가 떠 있을 때 · `?noauto=1` · 뒤로 가기 · 보이지 않는 탭.
첫 번째가 특히 중요하다. 비밀번호를 틀려 폼이 되돌아온 참에 기기 창을 덮으면
무엇이 틀렸는지 읽지도 못한다.

**마지막 기기를 지우면 우선순위를 되돌린다**(`Profile.razor`). 안 되돌리면
로그인 화면이 열릴 때마다 반드시 실패하는 기기 확인을 열고, 그 증상은
「지우고 나니 로그인 화면이 이상해졌다」로 보여 원인과 멀다.

#### 지문 단추는 **아이디 칸을 믿지 않는다** (2026-09-24, 실제로 밟음)

서버는 아이디를 받으면 그 사람의 열쇠 목록(`allowCredentials`)을 함께 주고,
못 받으면 빈 목록을 준다. **빈 목록으로는 「기기가 스스로 들고 있는
열쇠」(discoverable)만 쓸 수 있다.** 등록할 때 `residentKey: "preferred"` 로
부탁하지만 **부탁이지 약속이 아니라**, 윈도우 Hello 처럼 안 들고 있기로 하는
기기에서는 아이디 없이 여는 길이 통째로 막힌다.

그런데 지문 단추가 읽던 아이디 칸에는 **「아이디 기억하기」가 채워 둔 값**이
들어 있다 — **지난번에 *비밀번호로* 들어온 사람**의 것이다. 한 브라우저로
다른 사람이 잠깐 로그인하고 나가면 그 뒤로 지문 단추가 그 사람의 열쇠를 찾고,
그 사람에게 패스키가 없으므로 후보가 비어 **반드시 실패한다.** 증상은
「어제까지 되던 지문이 안 된다」 하나라 원인이 아이디 칸으로 보이지 않는다.

> 한동안 안 드러난 것은 `LoginInput.Username` 기본값이 `"quristyle"` 로 박혀
> 있었기 때문이다(8163ebf2 → e9faa145 에서 걷었다). 그 값이 칸을 늘 채우고
> 있어서 그 계정의 지문만 언제나 후보에 들어갔다.

그래서 누구의 열쇠를 찾을지를 `passkey.js`(`pickOptions`)가 셋으로 정한다.

| | 어디서 | 비고 |
|---|---|---|
| 1 | 사람이 **직접 친** 아이디 | `theme.js` 가 채운 값은 `data-jsini-autofilled` 로 갈라 둔다 |
| 2 | 이 브라우저가 패스키로 들어가던 아이디 | `PASSKEY_USERNAME_<호스트>` |
| 3 | 아이디 없이 | 기기가 스스로 들고 있는 열쇠에 맡긴다 |

2 는 **등록한 자리**(`Profile.razor`)와 **그 열쇠로 실제 서명을 받아낸
자리**(`passkey.js`)가 함께 적는다. 앞의 것만으로는 이 코드가 생기기 전에
등록해 둔 사람이 영영 빠지고, 뒤의 것만으로는 등록하자마자 한 번은 아이디를
쳐야 한다.

1·2 로 물어 후보가 비면 3 으로 **한 번 더** 묻는다. 게이트웨이가 이 경로도
분당 10회로 조이므로(`auth-attempts`) 왕복을 함부로 늘리지 않는다 — 2 가
빈손이면 적어 둔 아이디를 그 자리에서 지워, 다음부터는 왕복이 도로 하나다.

**후보가 하나도 없이 물었다가 거절당하면 문구가 달라진다.** 규격상 「취소」와
「맞는 열쇠가 없다」를 구분할 수 없는데, 「취소되었거나 시간이 지났습니다」로만
말하면 취소한 적 없는 사람이 그 글을 읽고 같은 단추를 다시 누른다. 그 자리에는
**할 일만 적는다** — 아이디를 한 번 치면 그 아이디가 이 브라우저에 남으므로
다음부터는 칠 일이 없다.

#### 잠금화면도 지문으로 푼다 — **로그인 경로를 재사용하지 않는다** (2026-09-25)

잠금화면(`LockScreen`)에 「지문 · 얼굴로 잠금 해제」가 섰다. 손쉬운 길은
`/webauthn/login` 을 다시 부르는 것인데 **두 가지가 깨진다.**

| | 무슨 일이 나나 |
|---|---|
| 새 토큰이 발급된다 | 잠금 해제가 조용히 **재로그인**이 된다. 비밀번호판을 `/user/verify-password` 로 따로 뺀 것과 같은 까닭이다 |
| 그 경로는 **익명이다** | 이 기기에 등록된 열쇠면 **누구 것이든** 통과한다 — 잠근 사람이 아닌 **옆 사람의 지문으로 화면이 열린다** |

그래서 확인만 하는 짝을 새로 뒀다.

| 경로 | 누가 | 무엇을 준다 |
|---|---|---|
| `auth/webauthn/verify/options` | 로그인한 사람 | 도전값. `allowCredentials` 는 **내 열쇠만**이고, 도전값에 **주인이 적힌다** |
| `auth/webauthn/verify` | 로그인한 사람 | 참·거짓. 토큰은 바뀌지 않는다 |

막는 자리가 셋이다 — 도전값의 주인 ≠ 지금 사람이면 거절, 서명한 열쇠의 주인
≠ 지금 사람이면 거절, 그리고 **실패 까닭은 기록에만** 남기고 화면에는 거짓만
준다(까닭을 갈라 주면 잠긴 화면 앞에서 남의 기기를 골라내는 데 쓰인다).

**틀렸을 때 401 이 아니라 200 + false 인 것**도 비밀번호판과 같다. 401 을 주면
`AuthTokenHandler` 가 세션이 죽은 것으로 읽고 토큰을 버려 **한 번 실패에 화면
전체가 로그아웃된다.** 시도 제한도 로그인과 같은 통(`auth-attempts`)이라,
게이트웨이에 경로 둘을 명시해 두지 않으면 `auth-route` 가 제한 없이 받는다.

**스스로 열지는 않는다.** 로그인 화면은 우선순위가 「지문 먼저」면 뜨는 순간
기기 확인을 여는데, 잠금은 **자리를 비우려는** 동작이라 잠그자마자 뜬 기기
창은 아무도 없는 앞에서 꺼지고 돌아온 사람은 실패 문구부터 읽는다.
우선순위가 잠금화면에서 바꾸는 것은 **차례와 단추 무게뿐**이다.

**등록된 기기가 있는지 미리 묻지 않는다.** `LockScreen` 은 로그인한 모든
화면에 얹혀 있어서, 미리 물으면 아무도 잠그지 않는 날에도 사람마다 왕복이
하나씩 는다. 단추는 브라우저 지원 여부(`jsiniPasskey.status`)로만 세우고,
눌렀을 때 후보가 비어 있으면 **기기를 부르지 않고** 그 자리에서 접는다.

#### 잠금은 **앱을 껐다 켜도 버틴다** — 걷는 자리는 로그인 화면 하나다 (2026-09-25)

잠금 표시가 `sessionStorage` 에 있었다. 새로고침은 버텼는데 **앱을 종료하고
다시 실행하면 덮개가 걷혀 있었다** — 세션 저장소는 문서가 닫히면 사라지고,
PWA 로 설치한 앱을 내리는 것이 곧 그것이다. 그런데 **앱을 내리는 것은 자리를
비우는 흔한 방법이지 잠금을 푸는 방법이 아니다.**

그래서 `localStorage` 로 옮겼다(`PortalBoot.ScreenLockedKey` — 읽는 자리도
`LocalKeys` 로 함께 옮긴다. 한쪽만 고치면 **잠기기는 하는데 되살아나지
않는다**).

| | 전 | 지금 |
|---|---|---|
| 새로고침 | 잠긴 채 | 잠긴 채 |
| 탭을 닫았다 다시 열기 | **풀린다** | 잠긴 채 |
| 앱 종료 → 다시 실행 | **풀린다** | 잠긴 채 |
| 로그아웃 → 다시 로그인 | 풀린다 | 풀린다 (아래) |

**기기에 남기면 걷어 줄 길목이 하나 필요하다.** 그 길목이 로그인 화면이다 —
`Login.razor` 가 `jsiniLock.forget` 을 부른다(공지 닫힘 표시를 거기서 지우는
것과 같은 자리·같은 까닭이다). 안 지우면 잠금화면에서 **로그아웃한 사람이 같은
비밀번호를 연달아 두 번** 치고, 남의 기기를 빌린 사람에게는 자기가 잠근 적
없는 덮개가 뜬다.

**로그인한 채로 `/login` 을 연 경우에는 지우지 않는다.** 그 화면은 이미
로그인한 사람을 되돌려 보내지 않으므로, 가리지 않고 지우면 **주소창에
`/login` 을 한 번 찍었다 돌아오는 것이 잠금 해제**가 된다. 이 잠금이 보안
경계는 아니지만(`ScreenLock` 머리말) 개발자 도구보다 쉬운 우회를 일부러
내줄 까닭은 없다.

> **잠금이 「탭의 것」에서 「기기의 것」으로 바뀌었다.** 이미 열려 있는 다른 탭은
> 그대로지만(저장소 사건을 듣지 않는다), 그 탭을 새로고침하거나 새 창을 열면
> 잠겨 있다. 한 사람이 쓰는 기기에서 그것이 맞는 쪽이다 — 잠가 놓고 새 탭을
> 열면 열리는 잠금은 잠금이 아니다.
...
> 것이고 이 값들은 **탭·브라우저의 것**이다(고정 탭은 창마다 다르고, 공지를
> 닫은 표시는 탭마다 다르다).
...
### 로그인한 뒤 **설치와 구독을 함께 권한다** (`PushAskPopup`)

등록된 기기가 **0 대**였다. 기능은 다 있는데 등록이 개인 환경설정 화면에
있고 거기까지 스스로 찾아가는 사람이 없었던 것이다. 그래서 레이아웃
(`MainLayout`)이 로그인한 뒤 묻는다 — 「앱 설치」·「알림 받기」를 누르면 그
클릭에서 설치 창과 권한 요청이 **그 자리에서** 이어진다.

권하는 것이 둘이다. **둘 다 휴대폰에서 값을 한다** — iOS 는 홈 화면 앱이라야
푸시를 주고, 안드로이드도 앱이라야 창을 닫아 둔 동안 온다.

#### 휴대폰에서는 되풀이하고, 데스크톱에서는 한 번만 묻는다

여태는 「나중에」 한 번이면 그 탭에서 끝, 「다시 묻지 않기」면 영영 끝이라
**정작 알림을 받아야 하는 기기에서 권유가 한 번 스치고 사라졌다.**

| | 휴대폰 | 데스크톱 |
|---|---|---|
| 「나중에」 | **15분 뒤 다시 뜬다**(`jsini-push-ask-snooze`) | 이 탭에서 그만(`jsini-push-ask-closed`) |
| 「다시 묻지 않기」 | **단추를 두지 않는다** | 영영 그만(`jsini-push-ask-never`) |
| 멈추는 때 | 설치와 구독이 **둘 다 끝났을 때** | 구독했을 때 |
| 옛 표시 | **보지 않는다** — 전에 「다시 묻지 않기」를 누른 휴대폰에도 다시 뜬다 | 그대로 본다 |

접어 둔 기한은 **기기에 남는다**(localStorage). 탭에만 남기면 홈 화면 앱을
껐다 켜는 것만으로 기한이 사라져 열 때마다 뜬다. 기한이 지나면 창을 열어 둔
채로도 다시 뜨는데, **훑지 않고 그 시각에 한 번 깬다**(`Task.Delay` 하나).

데스크톱은 건드리지 않았다 — 거기서는 탭으로 써도 알림이 오고, 앱 설치 하나
때문에 여태 안 뜨던 자리에 창이 새로 뜨지 않는다.

#### 설치는 세 갈래고, 단추를 놓을 수 있는 것은 한 갈래뿐이다

`jsiniPwa.status` 의 `installHint` 가 갈라 준다.

| 값 | 창이 하는 일 |
|---|---|
| `prompt` | **「앱 설치」 단추.** 누르면 그 자리에서 설치 창이 뜬다 |
| `ios` | 사파리다. 단추를 놓을 길이 없어 **「공유 → 홈 화면에 추가」를 적어 준다** |
| `menu` · `none` | **아무 말도 하지 않는다** |

마지막 줄이 중요하다 — 신호를 안 주는 브라우저와 **이미 설치해 둔** 브라우저가
거기서 구분되지 않는다. 설치한 사람에게 「메뉴로 설치하세요」를 되풀이하면
그것은 권유가 아니라 고장이다.

**설치를 마쳐도 보고 있던 탭은 탭 그대로다** — `display-mode` 는 `browser` 이고
한 번 쓴 설치 신호는 사라진다. 그 둘만 보면 방금 설치한 사람이 「아직 안 했다」로
읽혀서 또 권하게 되므로, `pwa.js` 가 `installed` 를 따로 찍는다.
...
|---|---|
| 탭이 잠겨 있지 않다 | 잠금화면 표시 |
| 접어 둔 기한이 지났다(휴대폰) | localStorage `jsini-push-ask-snooze` |
| 「나중에」·「다시 묻지 않기」를 안 눌렀다(데스크톱) | sessionStorage · localStorage |
| 설치 또는 구독에 **할 일이 남았다** | `jsiniPwa.status` |
| 서버가 보낼 수 있다(`pushAvailable`) | `preferences/me` |
| **스스로 끈 적이 없다**(`Saved && !PushEnabled`) | 〃 |

마지막 두 줄은 「구독은 빼고 설치만 권한다」로 갈라지는 자리다 — 스위치를
스스로 끈 사람에게 다시 구독을 권하면 조르기이지만, 앱으로 설치하는 것은
알림과 상관없이 값을 한다.

저장소 살핌은 **레이아웃이 이미 내는 왕복에 실려 온다**(`PortalBoot` 에 열쇠를
보탠 것이 전부다). JS 상태는 화면 전환마다 하나, **설정 조회는 구독을 권할
때만** 나고 둘 다 끝나면 사라진다.
...
### 오래 안 켜다 켜면 알림이 **한꺼번에** 쏟아진다 (2026-09-24)

웹푸시는 서버가 브라우저로 바로 꽂는 것이 아니다. 우리가 보낸 것은 브라우저
제조사의 푸시 서비스(크롬이면 FCM)가 받아 **연결이 살아날 때까지 들고 기다린다.**
그래서 사람이 보는 「온 시각」은 보낸 시각이 아니라 **배달된 시각**이고, 며칠
닫아 둔 브라우저를 여는 순간 그 줄이 통째로 흘러나온다.

막는 손잡이가 규격(RFC 8030)에 셋 있고 **하나도 안 보내고 있었다** — 그러면
라이브러리 기본 `TTL` 인 **28일**이 나간다.

| | 무엇을 | 어디서 정하나 |
|---|---|---|
| `TTL` | 지나면 푸시 서비스가 **버린다** | `Push:DefaultTtlSeconds`(6시간) · 알림별 `TtlSeconds` |
| `Topic` | 대기줄에서 **앞의 것을 밀어낸다** | `PushMessageDto.Topic`(비우면 `Tag`) |
| `Urgency` | 절전 중에도 미루지 않는다 | `Push:Urgency`(`high`) |

**버려도 되는 까닭**은 「내 알림함」(`/admin/push/history`)이 있기 때문이다.
푸시는 지금 알려 주는 길일 뿐이고 기록은 알림함에 남는다.

**`Topic` 은 `Tag` 와 다른 물건이다.** 태그는 브라우저가 **이미 받은** 알림창을
겹쳐 그리는 것이고, Topic 은 브라우저가 꺼져 있는 동안 **푸시 서비스가 들고
기다리는 줄**에서 겹친다. 배포 알림처럼 「켜져 있을 때는 각각 보이고 밀려
있을 때는 마지막 하나면 되는」 알림이 있어서 둘을 따로 둔다.

#### 늦게 온 것은 `push-sw.js` 가 한 줄로 묶는다

TTL 은 **부탁이지 보장이 아니라서** 기기가 절전에서 깰 때 시효 지난 것이 섞여
온다. 페이로드에 `sentAt` · `expiresAt` 이 실려 오고, 시효가 지났으면 낱낱이
띄우는 대신 **「지난 알림 N건」 하나**로 묶는다(누르면 알림함).

**버리지 않는 까닭**: push 이벤트를 받고 알림을 하나도 안 띄우면 크롬이 대신
"이 사이트가 백그라운드에서 갱신되었습니다" 를 띄우고, 반복되면 구독을 끊는다.

세는 값은 **떠 있는 묶음 자신**에게서 가져온다(`getNotifications`) — 서비스워커는
푸시 사이에 죽었다 살아나므로 변수에 들고 있으면 0 으로 돌아간다.
기기 시계 여유 2분이 없으면 **방금 온 알림이 시계 차이만큼 「지난 것」으로 접힌다.**

자세한 것은 [docs/push-delivery.md](../docs/push-delivery.md).
...
(2026-09-24 에 `sentAt` · `expiresAt` 둘이 늘었다 — 바로 위 절 참고.)
...
알림 설정 판에 **「내 위치 날씨」** 가 붙었다. 브라우저가 준 위경도의 지금
날씨와 예보를 **정해진 시각마다** 앱 푸시로 보낸다.

**같은 판의 「기상 특보」와 다른 스위치다.** 저쪽은 회사가 등록해 둔 지점
(`ghub.weather_locations`)에 기상청 특보가 걸렸을 때만 울리는 **사건** 알림이고,
이쪽은 내가 선 자리의 날씨를 **사건이 없어도** 보낸다. 한 칸에 얹으면 특보만
받고 싶은 사람이 매일 아침 알림을 함께 받는다.

#### 웹은 뒤에서 위치를 못 읽는다 — 그 한 가지가 구조를 정했다

서비스워커에는 `geolocation` 이 없고, 탭이 닫힌 뒤에 묻는 길도 없다. 그래서
**화면이 열려 있을 때 한 번 받아 서버에 저장해 두고**, 알림은 서버가 그
저장된 좌표로 만들어 보낸다. 이사·출장으로 자리가 바뀌면 사람이 다시 눌러야
하고, 설정 화면이 「잡은 때」를 보여 주는 것이 그 때문이다.

| 자리 | 하는 일 |
|---|---|
| `Shell/wwwroot/js/geo.js` | `jsiniGeo.locate()` — 브라우저에 위치를 묻는다 |
| `Components/Settings/NotificationPanel.razor` | 위치를 잡아 저장하고 **미리보기**를 보여 준다 |
| `scom.notification_preferences.weather_*` | 좌표 · 지역 이름 · 받을 시각 · 마지막 발송 |
| LifeEnvServer `GET /weather/point?lat=&lon=` | 좌표 하나의 지금 날씨 · 예보 · 지역 이름 |
| LifeEnvServer `LocalWeatherNotifyService` | 5분마다 깨어나 고른 시각에만 보낸다 |

#### 보내는 쪽이 생활과환경인 까닭

알림 서비스는 **누가 켰는지만** 안다. 어느 격자로 물을지(위경도 → `nx`·`ny`),
기상청 코드값(`SKY`·`PTY`)이 무슨 날씨인지, 그것을 알림 두 줄로 어떻게 줄일지는
전부 날씨를 아는 쪽의 몫이다. 그래서 **목록을 받아 가** 문장을 만들어 **돌려준다** —
`GET /weather-local/subscribers` → `POST /weather-local`. 방향은 기존 기상
이벤트와 같다(LifeEnv → Notification). 뒤집으면 알림 서비스가 기상청을 알게 된다.

> **대상 목록 엔드포인트만 신원을 한 번 더 본다.** 나가는 것이 사람들의 위경도라,
> `X-User-Id` 가 `system:` 으로 시작하지 않으면 403 이다. 게이트웨이를 지난 보통
> 계정에게 열어 두면 로그인한 누구나 동료가 어디 사는지 받아 갈 수 있다.

#### 걸려 본 곳 둘

- **봉투가 한 겹 더 있다.** `ApiResponse` 는 목록을 `data.result` 로 싸는데
  (`BuildSerializedData`) 받는 쪽을 `data` = 배열로 적어 두었더니 역직렬화가
  통째로 던졌고, 로그에는 「NotificationServer 가 꺼져 있을 수 있다」만 남아
  **서버가 멀쩡히 200 을 주는데도 꺼진 것처럼 보였다.**
- **「잡은 때」를 무조건 찍으면 안 된다.** 화면은 스위치 하나를 눌러도 설정
  전체를 보내므로(`ToggleAsync`), 저장할 때마다 찍으면 푸시 스위치를 만질
  때마다 위치를 방금 잡은 것이 된다 — 좌표가 **실제로 달라졌을 때만** 찍는다.

#### 격자 변환은 계산이고, 지역 이름만 표에서 찾는다

`GridConverter` 가 기상청 `dfs_xy_conv`(람베르트 정각원추)를 그대로 옮긴 것이다.
`ghub.grid_coordinates` 3834 행의 위경도를 넣어 보면 **3789 행이 저장된 `nx`·`ny`
와 정확히 같고** 43 행이 한 칸 차이(중심점을 소수점에서 반올림한 탓), 나머지
둘은 위경도가 `0` 인 빈 줄이다. 그 표는 **사람에게 보여 줄 이름**에만 쓴다 —
역지오코딩 서비스를 하나 더 물리면 그쪽이 죽었을 때 날씨 알림이 함께 멎는다.

**기상청 인증키(`Weather:ServiceKey`)가 없으면 알림이 나가지 않는다.** 발송기가
「기상청 자료 없음」으로 건너뛰고 시각 칸도 안 찍으므로 다음 깨어남에서 다시
시도한다 — 자료 없는 빈 알림을 보내는 것보다 낫다. 개발 장비에는 그 키가 없다.

### 위치는 묻고 나서, 그다음부터는 저절로 따라간다 (2026-09-24)

위의 구조에는 구멍이 둘 있었다. **아무도 위치를 주지 않는다**(잡는 단추가 개인
환경설정 안에 있다 — 알림 구독이 여태 0 대였던 것과 같은 자리다), 그리고
**한 번 준 좌표가 영영 그 자리에 머문다**(이사·출장 뒤에도 옛 동네 날씨가 온다).

| 자리 | 하는 일 |
|---|---|
| `Shell/wwwroot/js/geo.js` | `locate()` 물어서 잰다 · `quiet()` 물음창 없이 잰다 · `permission()` 사정을 본다 |
| `Components/Settings/GeoLocator.cs` | **절차 한 벌** — 재고 · 움직였나 보고 · 저장하고 · 이름을 채운다 |
| `Components/Layout/LocationAskPopup.razor` | 안 준 사람에게 **한 번 묻고**, 준 사람은 **조용히 확인**한다 |
| `scom.notification_preferences.weather_synced_at` | 마지막으로 **확인한** 때(좌표가 그대로여도 찍힌다) |

#### 묻는 창은 「이미 알림을 받는 사람」에게만 뜬다

위치를 저장하는 까닭이 **날씨 알림 하나**다. 구독하지 않은 사람에게 물으면
받아 두고도 쓸 데가 없고, 같은 화면에서 `PushAskPopup` 이 먼저 묻고 있는
중이기도 하다. 그 창에서 「알림 받기」를 누른 사람이 **다음번에 포털을 열 때**
이 창을 본다 — 물음창 둘이 한 화면에 겹치지 않는 것은 덤이다.

권한 요청은 **창의 단추에서** 일어난다. 화면이 뜨자마자 `getCurrentPosition` 을
부르면 아무도 부르지 않은 물음창이 튀어나오고, 사람은 그것이 무엇을 위한
것인지 모른 채 「차단」을 누른다 — **한 번 차단으로 굳으면 다시 물을 수 없다.**

#### 자동 재수집은 **탭이 열려 있는 동안**의 이야기다

서비스워커에는 `geolocation` 이 없어 창이 닫힌 뒤에는 어떤 길로도 위치를 못
...
- **한 시간에 한 번만 잰다**(`GeoLocator.SyncInterval`). 업무를 옮길 때마다
  레이아웃이 새로 만들어지므로(`PortalBoot` 머리말) 그대로 두면 하루에 수십 번
  잰다. 그 문턱은 **서버를 부르기 전에** 브라우저 저장소로 거른다
  (`jsini-geo-synced-at`).
  처음에는 세 시간이었다(2026-09-24 에 한 시간으로 줄였다). 알림 시각은 사람이
  고르는데 「세 시간마다 (6~21시)」를 고르면 **발송 간격과 확인 간격이 같아져**,
  낮에 자리를 옮긴 좌표가 다음 발송에 못 대고 한 칸 늦게 따라왔다. 한 시간이면
  가장 촘촘한 발송 간격에서도 **발송 전에 적어도 두 번**은 확인한다.
- **300m 넘게 움직여야 갈아 끼운다**(`MoveThresholdMeters`). 기상청 격자가 5km
  칸이지만 **칸 경계에 사는 사람** 때문에 문턱을 그보다 훨씬 낮게 둔다. 문턱이
  아예 없으면 GPS 가 가만히 있어도 흔드는 몇십 미터마다 「잡은 때」가 갱신되어
  그 값이 이사를 가리는 구실을 못 한다.
- **Permissions API 가 없는 브라우저**(옛 iOS 사파리)에서는 조용한 확인이 돌지
  않는다. 물음창이 뜰지 어떨지 알 수 없어서다 — 그 브라우저의 사람은 설정
  화면의 단추로 다시 잡는다.

#### 「잡은 때」와 「확인한 때」를 가른다

한자리에 사는 사람은 **좌표가 안 바뀌므로** 「잡은 때」가 몇 달 전 그대로다.
그렇다고 위치가 묵은 것은 아닌데, 칸이 하나뿐이면 화면이 그 둘을 갈라 말할 수
없다. 그래서 칸을 하나 더 두었다(`weather_synced_at`).

**아무 저장에나 찍으면 안 된다.** 설정 화면은 스위치 하나를 눌러도 설정 전체를
보내므로 위경도가 늘 함께 온다 — 좌표가 왔다고 찍으면 푸시 스위치를 만질 때마다
방금 확인한 것이 된다(「잡은 때」에서 이미 밟은 함정이다). 요청에
`weatherLocated: true` 가 실렸을 때만 찍는다.

> **뒤에서 도는 저장은 설정 전체를 보내지 않는다.** 조용한 확인이 전체를
> 보내면 다른 탭에서 방금 바꾼 스위치를 되돌린다 — 좌표만 싣는 길이 따로 있다
> (`NotificationClient.SaveMyLocationAsync`). 같은 까닭으로 **좌표만 온 저장은
> 지역 이름을 지우지 않는다**(자리가 바뀌었을 때는 반대로 지워야 한다).

DB: `deploy/sql/notification-weather-synced-2026-09-24.sql` (운영 반영 완료)

### 그 좌표를 한 장에 모아 보는 화면 (2026-09-25)

「위치 지도」(`/admin/location/map` · `admin.location.map`)가 **위치를 허용한
계정 전부**를 점으로 찍고, 점을 누르면 **그 자리에서 쪽지 창**이 뜬다
(`NoteWritePopup.DefaultTo`). 화면을 옮기지 않는 까닭은 상단 띠의 ✉ 와 같다 —
옮기면 누구를 보다가 눌렀는지가 사라지고 받는 사람을 손으로 다시 적게 된다.

**좌표를 새로 받지 않는다.** 출처는 위에 적은 그 칸 하나뿐이다
(`scom.notification_preferences.weather_lat`). 자리를 적는 칸이 둘이 되면
한쪽만 갱신되는 날이 반드시 오고, 그때 어느 쪽이 맞는지 아무도 말할 수 없다.
그래서 **목록에 없는 사람은 위치를 허용하지 않은 사람**이고, 그것이 「왜 저
사람이 안 보이나」의 유일한 답이다.

| | 설정 화면의 지도 | 위치 지도 |
|---|---|---|
| 부품 | `Settings/LocationMap.razor` | `Admin/wwwroot/js/geo-map.js` |
| 그리는 법 | OSM 끼워 넣기 판(iframe) | 타일 `<img>` 를 우리가 깐다 |
| 점 | **하나** (`marker=`) | 여럿 |
| 누를 수 있나 | 못 한다 (iframe 안쪽) | 누르면 쪽지 |

**끼워 넣기 판을 못 쓴다.** `export/embed.html` 은 `marker=` 를 하나만 받고,
iframe 안쪽은 우리 문서가 아니라 **점을 눌렀는지 알 길도 없다.** 그렇다고
Leaflet·카카오·네이버를 들이지도 않았다 — 필요한 것은 타일을 깔고 · 끌고 ·
확대하고 · 점을 누르는 것 넷뿐이고, 그 넷은 200줄이다. 열쇠(API 키)를 배포에
싣거나 CDN 한 곳을 더 매달 값어치가 없다. 타일 출처는 그대로 OSM 이다.

- **끌기·확대는 브라우저 안에서 끝낸다.** 회로로 넘기면 마우스를 움직일 때마다
  왕복한다 — 조직도 캔버스(`org-chart.js`)와 같은 까닭이다. C# 이 시키는 것은
  「점을 갈아 끼워라」 · 「맞춰라」 · 「저기로 가라」 셋뿐이고, 점을 누른 것만
  `[JSInvokable] PickMarker(key)` 로 되돌아온다.
- **같은 자리에 선 사람들을 벌려 놓는다.** 한 사무실에 앉으면 좌표가 소수점
  넷째 자리까지 같다. 그대로 찍으면 **맨 위 한 사람만 누를 수 있고** 나머지는
  영영 묻힌다. 작은 원으로 벌리고 원래 자리로 선을 긋는다.
- **「전체 보기」가 한자리에 모인 사람들 때문에 18 단계까지 당겨지지 않는다.**
  좌표 차이가 0 이면 어느 단계에서도 들어와서, 막지 않으면 건물 하나만 보인다.
- **쪽지가 안 닿는 사람을 빼지 않는다.** 쪽지 찾기 목록은 그런 사람을 빼지만
  (`NoteRecipientResolver.SearchAsync`), 지도가 묻는 것은 「누가 어디 있나」라
  쪽지와 무관하다. 점을 흐리게 두고 단추만 잠근다 — 빼 버리면 **사라진 까닭**을
  화면 어디에서도 알 수 없다.
- **메뉴 권한을 관리자 둘에게만 준다.** 직원 전체가 지금 어디 있는지는 협력사
  역할에 줄 것이 아니다 — 쪽지 메뉴들이 「내 알림함」의 권한을 베낀 것과
  갈라지는 지점이다.

#### 휴대폰에서는 판이 **한 번에 하나**다 (2026-09-25)

좌우 분할을 그대로 좁은 화면에 가져가면 `.ad-split` 이 위아래로 쌓아서
**표가 화면을 통째로 먹고 지도는 스크롤 저편**에 있다. 지도를 보러 온 사람이
매번 표를 지나 내려가야 하고, 내려가면 이번에는 조회 칸이 위로 사라진다.

≤767px 에서는 **보기 고르개**(지도 / 목록 N)를 두고 판을 하나씩 세운다.

| | 넓은 화면 | 휴대폰(≤767px) |
|---|---|---|
| 배치 | 좌우 분할 | 고르개 + **한 판** |
| 목록 | `CommGrd` (표) | 한 줄짜리 카드(`ad-geolist`) |
| 지도 높이 | 남은 높이 | **남은 높이** (바닥 360px 가 아니다) |
| 찾는 칸 | 조회 판 안 | 조회 판 + **목록 위에 한 번 더** |
| 카드 | 왼쪽 아래 17rem | 판 아래를 **가로로 채운다** + 닫기 × |

- **마크업을 갈아 끼우지 않는다.** 지도 판(`@@ref="_stage"`)은 늘 그려져 있고
  감추는 일은 CSS 가 한다(`.ad-split--list > .ad-split__main`). 렌더에서
  빼면 손잡이가 죽은 DOM 을 가리키고, 돌아올 때 지도를 처음부터 새로
  만들어야 한다 — 셸 사이드바가 `@@if (_isPhone)` 로 갈랐다가 물린 것과
  같은 자리다.
- **감춰진 판은 0 × 0 이라 아무것도 셀 수 없다.** 그동안 「전부 보이게
  맞춰라」를 셈하면 `best` 가 최소 단계로 떨어져 **지구 전체**가 나온다.
  `geo-map.js` 가 자리가 없으면 `pendingFit` 에 적어 두었다가 판이 다시 서는
  순간(`show()` · `ResizeObserver`)에 그때의 크기로 셈한다.
- **목록에서 사람을 누르면 지도로 넘어가며 그 자리로 옮긴다.** 누르고 나서
  고르개를 또 눌러야 한다면 목록을 쓸 이유가 없다. 그 길은 `PickAsync` 를
  거치지 않는다 — 그쪽은 **이미 고른 줄이면 아무것도 하지 않으므로**, 지도를
  끌어 옮겨 둔 뒤 같은 사람을 다시 눌러도 화면이 안 돌아온다.
**재어 본 값이다**(모형 한 장 + 헤드리스 크롬, 390px 폭 · 본문 740px).
눈대중이 아니다 — 높이를 채우는 사슬은 한 곳만 끊겨도 바닥값으로 주저앉는데,
그때도 화면은 멀쩡히 그려져서 코드만 읽어서는 안 보인다.

| | 전 | 후 |
|---|---|---|
| 지도 판 | 360px (바닥값) | **558px** |
| 목록에 보이는 줄 | 표 머리줄·쪽나누기·아래 띠를 빼고 남는 만큼 | 515px ÷ 40px = **12줄** |
| 쪽 스크롤 | 표를 지나 내려가야 지도 | **0px** (판 안에서만 구른다) |
| 데스크톱(1280×900) | 좌우 분할 · 지도 664px | **그대로** |

- **찾는 칸이 둘인 것은 값이 둘이라는 뜻이 아니다.** 같은 `_keyword` 를 본다.
  조회 판은 휴대폰에서 접히는데(`CommSch` 머리말) 이 화면의 검색어는 치는
  즉시 목록이 줄어드는 **주 조작**이라 그것 하나 때문에 매번 펴게 된다.
  `Foldable="false"` 로 끄지 않는 까닭은 나머지 조건까지 늘 펴 두면 작은
  화면에서 그만큼이 또 사라지기 때문이다.
...
**프레임워크 기본 상자는 회로가 사라진 것을 확인한 자리에서 곧바로
`location.reload()` 를 한다.** 섀도 DOM 안이라 그 한 줄을 막을 방법이 없고,
사용자에게는 「다시 붙자마자 화면이 통째로 새로 뜨는 것」으로 보인다.
우리 상자는 그 자리에서 **무슨 일인지 말하고 다섯을 센 뒤에** 연다 —
멈추고 싶으면 「잠시 멈추기」다(아래 「거절당하면 스스로 연다」).
...
### 상자를 **기억해 두면 단추가 죽는다** (2026-09-25, 실제로 밟음)

증상은 **「상자는 뜨는데 새로고침이 안 눌린다」** 하나였다.

`reconnect.js` 가 기동할 때 `getElementById` 를 한 번만 해서 그 요소에 손을
걸어 두었다. 그런데 **향상된 이동(enhanced navigation)은 문서를 다시 파싱하지
않고 `<body>` 를 기워 맞추면서 이 `<div>` 를 통째로 갈아 끼운다.** 그러면
...
새로고침하고 나면 (문서가 새로 파싱되므로) 저절로 멀쩡해져서 재현조차 어렵다.
헤드리스 크롬으로 상자의 리스너를 세어 보고야 잡았다 — 향상된 이동 하나를
거치면 `components-reconnect-state-changed, click` 이 **(없음)** 이 된다.

| | 어떻게 걸었나 |
|---|---|
| 누름 | **`document` 에 한 번.** 누른 자리에서 상자를 찾는다 — 요소가 몇 번 갈려도 안 끊긴다 |
| 상태 변화 | 상자에 직접. `CustomEvent` 가 **거품이 일지 않아**(`bubbles: false`) 문서로는 못 받는다 — 그래서 `enhancedload` 마다 지금 있는 상자에 다시 건다 |

**프레임워크 쪽에도 같은 구멍이 있다.** `DefaultReconnectionHandler` 는 처음
끊긴 순간의 상자를 기억해 두고 다시 찾지 않는다 — 「끊겼다 이어짐 → 향상된
이동 → 다시 끊김」 순서에서는 **아무것도 안 뜬 채 화면이 굳는다.**
`rebind()` 가 기억해 둔 것이 지금 상자와 다르면 비워 다시 찾게 한다.

### 거절당하면 **스스로 연다** (2026-09-25)

한동안 **어디에서도 스스로 새로고침하지 않는 것**이 이 상자의 존재 이유였다.
그런데 거절당했다(`rejected`)는 것은 **하던 일이 서버에 없다**는 뜻이고,
그때 화면은 이미 죽어 있다 — 말만 하고 사람이 누르기를 기다리는 것은
죽은 화면 앞에 사람을 세워 두는 것이었다.

| 상태 | 무엇이 일어났나 | 우리가 하는 일 |
|---|---|---|
| `rejected` | 서버에 닿았는데 **회로가 없다**(재시작·15분 초과) | 다섯을 세고 **스스로 연다**. 「잠시 멈추기」로 멈출 수 있다 |
| `failed` · `resume-failed` | 서버에 **닿지도 못했다** | 스스로 열지 않는다. 30초마다 조용히 다시 이어 본다 |

다섯 초는 적어 둔 것을 옮겨 적을 틈이다. 닿지 못한 쪽에서 새로고침하면
브라우저 오류 화면이 뜨고 하던 것만 잃으므로 그쪽은 손대지 않는다.

### 상자는 **잠금화면보다 위다** (z-index 20500)

잠긴 화면은 서버가 있어야 풀린다(비밀번호·지문을 서버가 대조한다). 덮개
(`.jsini-lock`, 20000)가 위에 있으면 상자가 비쳐 보이기만 하고 단추는 덮개에
먹혀 **화면을 되찾을 길이 아예 없다.** 자리를 비운 사이에 끊기는 것은 가장
흔한 조합이기도 하다.
...
- **휴대폰에서는 그 판이 접힌다 — 화면이 켜는 것이 아니다.** 조건(`Fields`)이
  있는 `CommSch` 는 좁은 화면(≤767px)에서 모두 「조회 조건 ⌄」 한 줄로 접히고,
  조회를 누르면 다시 접힌다. 화면이 할 일은 **접힌 줄에 적을 글자**를 주는
  것 하나다 — `MobileSummary="@ConditionSummary"`(「전체 · 개발팀 · 9 월」).
  안 주면 머리줄에 「조회 조건」만 남아 무엇으로 걸러 본 목록인지 알 수 없다.
  값을 부품이 만들 수 없는 까닭(자식이 `RenderFragment` 다 · 「전체」를 무엇이라
  부를지가 화면마다 다르다)은 `CommSch` 머리말에 있다. 단추만 있는 조작줄은
  접지 않고(`Fields` 가 없다), 접는 줄이 오히려 자리를 먹는 화면은
  `Foldable="false"` 로 끈다. 데스크톱에서는 접기 자체가 없다.
  **빠뜨리면 아키텍처 테스트가 잡는다**(`CommSchFoldTests`) — 빠뜨려도 화면은
  멀쩡히 접히기 때문에 눈으로는 못 지킨다.
- **그 글자는 `SchSummary` 로 잇는다.** 조각을 가운뎃점으로 잇고(`Of`), 빈
  조각은 버리고, 안 고른 칸은 「전체」라 부른다(`Or` · `Any`). 스위치는 켰을
  때만 적고(`On`), 기간은 올해면 해를 뗀다(`Period` · `Day`). 화면마다
  `string.Join` 을 적으면 반드시 갈라진다 — 어떤 화면은 쉼표로 잇고 어떤
  화면은 빈 칸을 안 걸러 「 ·  · 9 월」이 된다.
- **고르개 목록에 익명 형식을 쓰지 않는다.** `new { Text, Value }` 는 화면에
  넘기는 데는 충분하지만 **값으로 이름을 되찾을 수 없어** 접힌 줄에 코드가
  적힌다(「y」 · 「edt」). 공용 `SchOption(Value, Text)` 을 쓰고
  `SchSummary.NameOf` 로 되찾는다.
- **목록이 부품 안에 있는 고르개는 이름을 올려 준다.** `CodeSelect` ·
  `SourceSelect` 는 `@bind-Text` 로 고른 항목의 이름을 화면에 준다
  (`@bind-Value` 는 코드만 준다). `BizSelect` 는 `OnChanged` 의 항목에서 받는다.
...
- **동작의 결과는 토스트다** — 저장·삭제·조회가 어떻게 됐는지는 `DataPage` 가
  ...
  **손가락으로는 옆으로 쓸어 치운다** — 닫기 단추가 손가락보다 작아 휴대폰에서
  잘 빗나가서다(theme.js·app.css 의 「쓸어 치우기」). 치우는 일은 그 닫기 단추를
  대신 눌러서 한다 — DOM 에서 직접 지우면 DevExpress 목록에 유령이 남아
  `MaxToastCount` 가 어긋난다. **마우스 끌기는 건드리지 않는다**(실패 문구를
  끌어서 고르고 복사하는 것이 쓰임새다).
...
- **그 안내 줄도 「화면에 이미 있는 것」을 다시 말하지 않는다** (2026-09-24).
  판정이 하나 더 있다 — 그 글을 지웠을 때 **사람이 못 하게 되는 일이 있나.**
  단추 글자 · 체크칸의 켜짐 · 탭 이름 · 글상자 자리글이 이미 말하는 것을
  줄로 한 번 더 적으면, 그 줄은 **매번 자리를 먹으면서 아무것도 더 알려 주지
  않는다.** 휴대폰에서 그 한 줄은 아래 내용을 그만큼 자판 밖으로 민다.

  | 지운다 | 남긴다 |
  |---|---|
  | 동작의 결과(「보냈습니다」·「저장했습니다」) | 막힌 상태 · 권한 없음 · 설정 빠짐 |
  | 단추·탭·체크칸이 이미 말한 것 | 화면 어디에도 없는 주의(「받는 쪽이 등록해야 도착합니다」) |
  | 왜 그런지의 구현 사정(「서버에 저장 통로가 아직 없습니다」) | 빈 판을 채우는 안내(「왼쪽에서 고르십시오」) |

  글상자의 자리글에 얹을 수 있으면 그쪽이 낫다 — **칸을 쓰는 순간에만**
  보이고 자리를 안 먹는다(`AiTaskActions` 의 창 둘이 그렇게 옮겼다).
- **조작법은 접어 둔다** — `HelpNote`(`<details>` 한 줄, ProjMng). 단축키처럼
  단추에 없는 동작은 적어 둘 데가 거기뿐이라 지울 수 없지만, 펼친 채로 달면
