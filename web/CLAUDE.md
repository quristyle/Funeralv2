...
## PWA — 설치와 웹푸시 (2026-09-13 복구)

포털은 **홈 화면에 설치되고 웹푸시를 받는다.** Vue 시절에는 vite-plugin-pwa 가
그 자리를 만들어 주었는데, 그 껍데기가 사라지면서 **매니페스트도 서비스워커도
통째로 빠진 채** 한동안 굴러갔다.

증상이 조용해서 특히 나빴다 — 화면은 멀쩡히 돌고, [알림 설정] 의 기기 목록만
영원히 비어 있었다. 새로 구독할 길이 없었으니 당연한데, 화면만 보면
"아직 기기를 안 붙였나 보다" 로 읽힌다.
...
서비스워커의 **범위(scope)는 파일이 놓인 경로 아래**다. Razor 클래스
라이브러리(`JSini.Web.Components`)에 두면 `/_content/...` 로 실려서 그 폴더
밖을 관장하지 못한다 — 알림을 눌러도 업무 화면으로 못 간다.

### 캐시하지 않는다. 그래도 `fetch` 처리기는 둔다

Blazor Server 는 화면을 서버가 그려 회로로 보낸다. 캐시해 둘 화면이 아예 없고,
어설프게 HTML 을 캐시하면 옛 프레임워크 파일을 가리키는 껍데기가 남아 회로가
안 붙는다. 그런데 브라우저가 "설치 가능" 으로 치려면 **fetch 를 듣는**
서비스워커가 있어야 한다. `push-sw.js` 의 빈 처리기가 그것이고,
`respondWith` 를 부르지 않으므로 아무것도 가로채지 않는다. **지우면 홈 화면
추가가 조용히 사라진다.**

### 순서가 정해져 있다

구독은 **브라우저에서 먼저 만들고 서버에 올린다.** 해제는 **서버를 먼저
지운다.** 둘 다 "한쪽만 남는" 방향을 피하려는 것이고, 남는 쪽이 늘
*서버에는 있는데 브라우저에는 없는* 죽은 구독이라 발송마다 실패가 쌓인다.

권한 요청(`Notification.requestPermission`)은 **사용자 클릭에서 이어져야**
한다. 화면이 뜨자마자 부르면 브라우저가 조용히 거절한다. 그래서 상태 읽기와
구독 만들기를 갈라 두었다 — 앞엣것만 자동으로 돌고 뒤엣것은 단추에 붙어 있다.

### 「왜 알림이 안 오는지」를 화면이 넷으로 나눠 말한다

`jsiniPwa.status()` 가 네 가지를 함께 돌려주고 알림 판(`NotificationPanel`)이
딱지 넷으로 세운다 — **실행 방식 · 서비스워커 · 알림 권한 · 이 브라우저 등록**.

한동안 「등록됐다 / 아직이다」 한 줄뿐이었는데, 안 오는 까닭이 넷이라 그
한 줄로는 무엇이 막혔는지 알 수 없었다. 나란히 두면 **막힌 칸이 곧 할 일**이다.

| 칸 | 어떻게 판정하나 | 거짓이면 |
|---|---|---|
| 실행 방식 | `display-mode` 미디어 질의 + `navigator.standalone`(iOS) | 데스크톱은 그래도 온다. **iOS 는 홈 화면 앱에서만 준다** |
| 서비스워커 | `getRegistration()` 의 `active` | 구독을 만들 수 없다 |
| 알림 권한 | `Notification.permission` | 단추를 눌러 받는다(차단은 브라우저 설정에서 푼다) |
| 이 브라우저 등록 | 구독 유무 | 스위치가 켜져 있어도 **안 온다** |

**`controller` 로 서비스워커를 보지 않는다.** 우리 워커가 `clients.claim()` 을
부르므로 곧 채워지지만 처음 연 문서에서 잠깐 비어 있어, 그것으로 판정하면
「서비스워커 없음」이 한 번 스친다.

**등록이 안 된 상태는 주의(Warning)로 말한다.** 이 화면에서 가장 자주 잘못
읽히는 자리다 — 스위치가 켜져 있으면 다 된 줄 알기 쉬운데 스위치는
「받겠다는 뜻」이고 받으려면 이 브라우저를 등록해야 한다.

**「앱으로 설치」 단추는 `beforeinstallprompt` 를 받아 둔 때만 보인다.**
그 사건은 한 번 지나가면 다시 오지 않아 `pwa.js` 가 기본 동작을 막고 들고
있다가 단추에서 쓴다. **없다고 설치가 안 되는 것은 아니다** — 이미 설치했거나
그 신호를 안 주는 브라우저(파이어폭스·사파리)다. 그때는 단추를 두지 않는다.
눌러도 아무 일이 없으면 그것은 고장으로 읽힌다.

### 로그인한 뒤 **한 번 권한다** (`PushAskPopup`)

등록된 기기가 **0 대**였다. 기능은 다 있는데 등록이 개인 환경설정 화면에
있고 거기까지 스스로 찾아가는 사람이 없었던 것이다. 그래서 레이아웃
(`MainLayout`)이 로그인한 뒤 한 번 묻는다 — 「알림 받기」를 누르면 그
클릭에서 권한 요청과 구독이 이어진다.

뜨는 조건이 여덟이고 **하나라도 어긋나면 조용히 넘어간다.**

| 살피는 것 | 어디서 |
|---|---|
| 이 탭에서 「나중에」를 안 눌렀다 | sessionStorage `jsini-push-ask-closed` |
| 「다시 묻지 않기」를 안 눌렀다 | localStorage `jsini-push-ask-never` |
| 탭이 잠겨 있지 않다 | 잠금화면 표시 |
| 웹푸시를 하는 브라우저다 · 권한이 차단이 아니다 · 아직 구독이 없다 | `jsiniPwa.status` |
| 서버가 보낼 수 있다(`pushAvailable`) | `preferences/me` |
| **스스로 끈 적이 없다**(`Saved && !PushEnabled`) | 〃 |

마지막 줄이 핵심이다 — 스위치를 스스로 끈 사람에게 다시 권하면 그것은
권유가 아니라 조르기다.

앞의 둘은 **레이아웃이 이미 내는 왕복에 실려 온다**(`PortalBoot` 에 열쇠 두
줄을 보탠 것이 전부다). 나머지 둘(JS 상태 · 설정 조회)은 **구독하지 않은
사람에게만** 나고, 구독을 마치면 사라진다.

### 구독 절차는 `PushEnroll` 한 벌이다

부르는 자리가 둘이다 — 알림 판의 단추와 이 권유 창. 그런데 순서를 틀리면
**조용히 깨지는 대목이 셋**이라(브라우저 먼저 · 서버가 안 받으면 되돌리기 ·
스위치 함께 켜기) 자리마다 한 벌씩 적으면 반드시 갈라진다. 절차는 서비스
하나에 있고 부르는 쪽은 결과를 받아 말만 한다.

### `navigator.serviceWorker.ready` 를 상태 읽기에 쓰지 않는다

그 약속은 **활성 워커가 생겨야** 풀리는데, 등록이 아예 없는 브라우저에서는
영영 풀리지 않는다(거절도 하지 않는다). 상태를 읽는 길이 거기서 멈추면
그 값을 기다리는 화면도 함께 멈춘다 — 권유 창이 그 값으로 뜰지 말지 정한다.
`getRegistration()` 은 없으면 `undefined` 로 풀리고, 구독을 읽는 데는 활성
워커가 필요 없다.

### 알림 세 화면의 출처가 바뀌었다 (2026-09-13)

「푸시 현황」·「발송 이력」·「내 알림함」이 **헬프데스크 DB** 를 읽고 있었다.
그 표에 쓰는 것은 헬프데스크 자신의 발송 코드뿐이라, 포털관리의 「메시지 발송」
으로 보낸 알림이 **세 화면 어디에도 안 나왔다.**

지금은 **보낸 쪽(NotificationServer)이 자기 기록을 갖는다** — `scom.push_send_logs`.
표는 SQL 로 만든다(`deploy/sql/notification-push-*.sql`) — 그 서비스에는
Migrations 폴더가 없고 `scom` 이력을 AuthServer·FileServer 가 함께 쓴다.

| 화면 | 무엇을 보나 | 엔드포인트 |
|---|---|---|
| 푸시 현황 | 보낸 쪽 통계 | `notification/notifications/push/stats·trend·failure-reasons` |
| 발송 이력 | 기기 단위 한 줄 | `…/push/logs` |
| 내 알림함 | **받은 사람 단위**(발송 한 번이 한 줄) | `…/inbox`, `…/inbox/{id}/read` |

**같은 표를 각도만 바꿔 본다.** 줄은 기기 단위라, 알림함은 `batch_id`(발송
한 번)로 묶어 낸다 — 안 묶으면 기기 둘 쓰는 사람에게 같은 알림이 두 줄로
보이고 하나만 읽음 처리하면 나머지가 남는다.

**못 보낸 것도 줄로 남는다**(구독한 기기 없음 · 본인이 푸시를 끔 · 구독 만료).
그 둘이 「왜 안 왔나」의 답이고, 그때는 발송 시도조차 없어 안 남기면 흔적이
아예 없다. 사유 글자는 `PushSender` 의 상수다 — 예외 메시지를 그대로 넣으면
같은 원인이 열 갈래로 흩어져 「실패 사유별 건수」가 뜻을 잃는다.

**도달·열람은 아직 헬프데스크 것이다.** 그것을 세려면 서비스워커가
「받았다·눌렀다」를 되돌려 줘야 하는데 포털 알림에는 그 길이 없다. 현황 화면은
그 셋을 **아래로 갈라 놓고 그 사실을 적어 둔다** — 같은 타일 줄에 섞으면
「보낸 건 100인데 도달이 0」으로 읽혀 없는 장애를 쫓게 된다.

> **통계 엔드포인트 하나만 `ApiResponse` 를 안 쓴다.** 그 봉투는 무엇을 담든
> `data.result` **배열**로 만들어서(`BuildSerializedData`), 값 한 벌을 받는
> 타일이 통째로 안 그려진다. 실제로 그래서 현황 타일 넷이 빈 채로 떴다.

### 서버 쪽은 손댈 것이 없었다

NotificationServer 에 이미 다 있다 — `GET /notifications/vapid-public-key`,
`POST|DELETE /notifications/subscriptions`, `GET /preferences/me`(공개 키와
기기 목록을 함께 준다). 페이로드 키(`title`·`body`·`url`·`icon`·`tag`)는
`PushSender.BuildPayload` 와의 약속이라 **바꾸면 알림이 빈 채로 뜬다.**

운영에는 VAPID 키가 들어 있고 **개발 장비에는 없다.** 없으면
`preferences/me` 가 `pushAvailable: false` 로 오고 화면이 그렇게 말한다 —
"스위치를 켜도 아무 일이 없다" 를 기기 문제로 오해하지 않게.

## 첨부 고르기 — `DxUpload` 과 `DxFileInput` 은 다르다

첨부는 `Components/Data/FilePicker.razor` 한 벌이고, 여섯 화면이 쓴다(공지 ·
자료실 · 영정 사진 · 미디어 · 헬프데스크 요청 · 메일). **생김새는 2026-09-13
부터 DevExpress `DxFileInput` 이 그린다** — 끌어놓기 칸 · 파일 목록 · 파일별
진행 막대 · 빼기 · 취소 · 잘못된 형식 표시가 전부 그쪽 것이다.

그 전에는 표준 `InputFile` 위에 그 전부를 직접 그렸다. 동작은 같았지만
포털의 다른 화면과 미묘하게 달랐고, 고칠 때마다 우리 CSS 를 손대야 했다.

### 갈리는 지점은 **바이트가 어디로 가느냐** 하나다

| | `DxUpload` | `DxFileInput` |
|---|---|---|
| 올릴 주소 | `UploadUrl` 필수 | **그런 파라미터가 없다** |
| 바이트가 가는 곳 | 브라우저 → 그 주소로 직접 POST | 브라우저 → **우리 회로**(`FilesUploading`) |
| 이 포털에서 | **쓸 수 없다** | 쓴다 |

**`DxUpload` 이 안 되는 이유는 취향이 아니라 구조다.** 이 포털은 BFF 라
브라우저에 게이트웨이 토큰이 없다(「인증 — BFF」). 브라우저가 게이트웨이로
직접 올리면 401 이고, 통하게 하려면 토큰을 브라우저로 내려보내야 하는데
그것이 이 구조를 세운 이유를 지우는 일이다.

`DxFileInput` 은 `InputFile` 과 같은 길이다 — 스트림을 서버 쪽 Razor 코드에
넘겨 주고, 거기에는 토큰이 있다.

### 고르는 즉시 임시 파일로 받아 둔다

스트림은 **그 상호작용 동안에만** 유효하다. 고른 뒤 폼을 채우다 저장을
누르면 그때는 이미 못 읽는 일이 생긴다 — 증상이 「어제는 됐는데 오늘은
안 된다」라 원인을 찾기 어렵다. 임시 파일은 부품이 사라질 때 지운다.

### 열쇠는 이름이 아니라 DevExpress 의 `Guid` 다

같은 이름을 두 번 고를 수 있고, 그때 한쪽을 빼면 **엉뚱한 임시 파일이
지워진다.** 빼기·총량 판정은 전부 그 `Guid` 로 짝을 짓는다.

### 개수·크기·형식은 DevExpress 가 막는다. **총량만 우리가 막는다**

`MaxFileCount` · `MaxFileSize` · `AcceptedFileTypes` 는 그쪽이 파일에 표시를
붙여 막아 준다. 우리가 세어 빼면 「왜 빠졌는지」가 토스트로만 남고 목록에서는
사라져서 같은 파일을 다시 고르게 된다.

**총량(`MaxTotalBytes`)은 DevExpress 에 그 개념이 없다.** 메일 첨부 때문에
생겼다 — 서버가 총량 15MB 로 막는데(`EmailEndpoints.MaxAttachmentBytes`) 화면은
한 개 크기만 보고 있었다. 안내에는 「모두 합쳐 15MB」라고 적혀 있었지만
**실제로 막지 않아서** 5개 × 15MB 를 다 받아 올린 뒤에야 서버가 거절했다.
먼저 말해 주려고 둔 상한이 먼저 말해 주지 못한 셈이다.

판정은 `SelectedFilesChanged` 에서 한다 — 그 이벤트는 파일이 서버로 넘어오기
**전**에 오므로, 넘치는 파일을 받기 전에 돌려보낼 수 있다.

### 이벤트 처리기에서 던지면 **회로가 끊긴다**

`RemoveFiles`/`RemoveAllFiles` 는 감싸 둔다. 첨부 하나를 목록에서 못 빼는 것과
화면 전체가 멎는 것은 무게가 다르다.

**`DisposeAsync` 에서는 디스크만 치운다.** 자식 부품의 목록을 비우거나
...
- **라벨을 붙인 칸에는 자리표시 글을 두지 않는다.** 조회 조건이든 편집
  폼이든 같다. 「검색어」 옆 칸에 「제목·내용」이, 「약칭」 옆에 「현황판에 쓰는
  짧은 이름」이 또 떠 있으면 라벨이 이미 한 말을 회색 글자로 한 번 더 하는
  것이고, 칸이 여럿 늘어선 판에서는 **어디부터가 사람이 적은 값인지** 한눈에
  안 들어온다. `NullText`(부품에 따라 `Placeholder`)를 걸지 않는다 —
  `CodeSelect`·`BizSelect`·`SourceSelect` 의 `Placeholder` 기본값도 빈 글자라
  화면이 적을 것이 없다. 라벨이 없는 칸(`CommSchItem` 에 `Label` 을 안 준 칸,
  도구줄의 검색 칸)은 자리표시 글이 라벨 노릇을 하므로 그대로 둔다.
  자리표시 글이 꼭 필요하면 그것은 라벨이 모자란다는 뜻이고, 그때는 라벨을
  고친다. 「비우면 999」·「최상위」처럼 **빈 값의 뜻**을 사람에게 알려야 하면
  칸 안이 아니라 칸 아래 `jsini-hint` 줄에 적는다.
- **동작의 결과는 토스트다** — 저장·삭제·조회가 어떻게 됐는지는 `DataPage` 가
...
- **토스트를 손으로 만들지 않는다.** `DxToastProvider` + `IToastNotificationService`
  ...
  고른 크기를 따로 좇아야 하고** 테마를 올릴 때 토스트만 옛 색으로 남는 쪽으로
  어긋난다. `Toasts` 는 그 서비스 앞에 세운 얇은 창구일 뿐이다 —
  `NoticeTone` 셋을 `ToastRenderStyle` 과 「실패는 얼마나 남는가」로 옮기는 규칙을
  한 곳에 두려는 것이고, 그 규칙이 흩어지면 화면마다 색과 시간이 갈린다.
- **사라지면 안 되는 안내는 `PageNotice`** 다 — 「왼쪽에서 메뉴를 고르십시오」 ·
  「이 화면은 조회만 합니다」처럼 **화면의 상태**를 적는 자리다. 가르는 기준은
  「방금 무슨 일이 일어났는가」(토스트)냐 「지금 이 화면이 어떤 상태인가」
  (안내 줄)냐다. 묶음 길잡이는 `GroupLinks`.
  > 잠금 덮개(`LockScreen`) 안에서는 토스트를 쓸 수 없다 — DevExpress 토스트
  > 판이 `z-index: 2500` 이고 덮개가 20000 이라 **덮개 뒤에 뜬다.**
...
- [ ] **첨부는 `FilePicker` 한 벌로 올린다** (`Components/Data/FilePicker.razor`).
      새 화면에 첨부를 붙일 때 이 부품을 쓴다. 아래 「첨부 고르기」 참고 —
      **`DxUpload` 과 `DxFileInput` 은 이름이 비슷하지만 갈리는 지점이 있다.**
