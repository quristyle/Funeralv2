...
개선요청/댓글/첨부, 공지, 회사·고객, 리포트, 웹푸시를 담당한다.

**조직 관리(팀·담당자)와 계정·메뉴·역할 권한은 여기 없다.** JSini 관리 포털
(AuthServer)이 단독으로 맡는다. 2026-09-25 에 헬프데스크의 「조직 관리」 메뉴와
`/api/admins` · `/api/teams` 묶음을 걷어냈다.
담당자·팀 **레코드**는 요청 배정이 참조하므로 그대로 둔다.

**프로젝트·WBS·일정도 여기 없다.** 같은 날 「프로젝트」(HD_PRJ)와
「일정」(HD_SCHEDULE) 메뉴 묶음을 걷어내면서 `/api/project` · `/api/schedules` ·
`/api/wbslink` 묶음 전부와 `/api/wbs` 의 평탄화·단건·쓰기,
`dashboard/project-stats/{projectId}` 를 지웠다. 남은 것은 `GET /api/wbs`(트리)
하나로, 「도구 > 다이어그램」 화면(`/helpdesk/util/diagram`)이 그림을 걸 항목을
고르는 데 쓴다. 프로젝트·WBS·일정 **레코드**와 표는 그대로 둔다.
...
포털 계정을 바로 사용하여 헬프데스크에서 사용하므로 이제 계정 연결 화면은 사용하지 않는다.
