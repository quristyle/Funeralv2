...
describe('storageManager', () => {
  ...
  it('should clear expired items', () => {
    storageManager.setItem('user', { age: 30, name: 'John Doe' }, 1000); // 1초 후 만료
    vi.advanceTimersByTime(1001); // 시간 진행
  ...
  it('should not clear non-expired items', () => {
    storageManager.setItem('user', { age: 30, name: 'John Doe' }, 10_000); // 10초 후 만료
    vi.advanceTimersByTime(5000); // 시간 진행
    storageManager.clearExpiredItems();
    const user = storageManager.getItem('user');
    expect(user).toEqual({ age: 30, name: 'John Doe' });
  });
  ...
  it('should handle multiple items with different expiry times', () => {
    storageManager.setItem('user1', { age: 30, name: 'John Doe' }, 1000); // 1초 후 만료
    storageManager.setItem('user2', { age: 25, name: 'Jane Doe' }, 2000); // 2초 후 만료
    vi.advanceTimersByTime(1500); // 시간 진행
    storageManager.clearExpiredItems();
  ...
  it('should handle items with no expiry', () => {
    storageManager.setItem('user', { age: 30, name: 'John Doe' });
    vi.advanceTimersByTime(10_000); // 시간 진행
    storageManager.clearExpiredItems();
    const user = storageManager.getItem('user');
    expect(user).toEqual({ age: 30, name: 'John Doe' });
  });
