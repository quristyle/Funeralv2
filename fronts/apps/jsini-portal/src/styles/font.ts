 ...
 *
 * [고정폭(`font-mono`)도 이 값을 따른다]
 * `internal/tailwind-config/src/theme.css` 의 `--font-mono` 가 고정폭 글꼴 뒤에
 * `var(--font-family)` 를 붙여 둔다. Tailwind 기본 고정폭 목록에는 한글 글리프가 없어서,
 * 명령어와 한글이 섞인 칸에서 한글만 엉뚱한 글꼴로 그려지던 것을 그렇게 잡았다.
 * 글꼴 대체는 글리프 단위라 라틴은 고정폭, 한글은 여기서 고른 글꼴이 된다.
 */
