...
const [Grid, gridApi] = useVbenVxeGrid({
  gridOptions: {
    columns: [
      { field: 'code', title: 'JQLSME 코드명', minWidth: 150 },
...
<template>
  <Page auto-content-height>
    <div class="mb-4 bg-card p-4 rounded border flex justify-between items-center">
      <div class="text-sm font-semibold">JQLSME 프로토콜 기기 연동 데이터 관리 센터</div>
      <Button type="primary" @click="gridApi.query()">전체 코드 동기화</Button>
    </div>
    
    <Grid table-title="JQLSME 연동 상태 및 파라미터 매핑 목록">
      <template #action="{ row }">
